...
class Classifier(object):
    ...
    def get_entropies(self, valid_pixels: torch.tensor, probas: torch.tensor,
        ...
        # entropy = ((1-(torch.pow(probas, self.alpha_values)).sum(2))/(self.alpha_values-1))
        entropy = self._valid_mean(entropy, valid_pixels, (1, 2, 3))
        ...
        d_kl = (torch.log(1e-10 +(torch.pow(marginal, self.alpha_values)*torch.pow(self.pi + 1e-10, 1-self.alpha_values)).sum(1))/(self.alpha_values-1))
